...
Decision: **Replace always-public registration with instructor-controlled registration links.**

Registration should only open through a link generated by an instructor/admin for a specific subject. Links can expire and can be manually closed.

Rules:
* `/register` should not accept direct public registration.
* Students must use `/register/:token`.
* The token controls the subject and allowed blocks.
* Instructors can generate links from the admin/instructor sidebar.
* Instructors can set an expiration date/time.
* Instructors can stop accepting registration by closing the link.
* If no blocks are selected for a link, all blocks in the subject are allowed.
* If blocks are selected, students can only register into those blocks.

---

## Registration & Export Upgrade

### Proposal Upload / File Storage
Status: **On hold.**

Proposal file upload should not be shipped in the current registration flow yet. The planned direction is to use object storage for files and keep MongoDB focused on structured system data.

Planned storage approach:
* Store proposal files in S3-compatible object storage.
* Bucket endpoint under review: `https://step-v2-bucket.s3.ap-southeast-1.amazonaws.com/`
* Do not store full proposal files as base64 inside MongoDB.
* MongoDB should only store proposal metadata and the storage key/reference.

Recommended future data shape:

```javascript
proposal: {
  fileKey: "proposals/IPT/22-ITE-08/group-id/proposal.pdf",
...
* Student selects a proposal file during registration.
* Backend validates the file request and creates a temporary pre-signed upload URL.
* Frontend uploads directly to the bucket using the pre-signed URL.
* Backend saves the proposal metadata and file key on the group record.
* Instructors can later open the proposal through a temporary signed download/view URL.

Security rules:
* AWS/S3 secret keys must stay only in the backend environment variables.
* The frontend must never receive permanent bucket credentials.
* Keep proposal files private unless there is a clear reason to make them public.
