    ...
    <div className="evl-card p-5">
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <p className="text-text/65 text-xs font-semibold mt-0.5">Max: {max} pts</p>
        </div>
        {level && (
      ...
      <div className="flex gap-2 flex-wrap mb-4">
        {levels.map((l) => (
          <button
            ...
            className={`flex-1 min-w-[80px] text-xs px-2 sm:px-3 py-2 rounded-lg border font-extrabold transition-all duration-150 ${l.color} border-transparent ${disabled ? 'opacity-60 cursor-not-allowed' : 'hover:scale-105 active:scale-95'}`}
          >
            {l.label}
          </button>
        ))}
      ...
      {level?.description && (
        <div className="mb-4 p-3 bg-muted/20 rounded-lg">
          <p className="text-xs text-text/70 leading-relaxed italic">{level.description}</p>
        </div>
      )}
      ...
      <div className="relative group">
        ...
        />
        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-text/55 group-focus-within:text-primary">/ {max}</span>
      </div>
    </div>
  );
}
